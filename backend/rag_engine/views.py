...
def upload_pdf(request):
    ...
    try:
        # === 1️⃣ Extract text and chunk ===
        text = extract_text_from_pdf(file_path)
        chunks = chunk_text(text)
        for i, c in enumerate(chunks):
            c["metadata"]["source"] = file.name

        # === 2️⃣ Build / update FAISS vector index ===
        build_faiss_from_chunks(chunks, persist=True)

        # === 3️⃣ Auto-delete uploaded PDFs to save space ===
        try:
            ...
            for f in os.listdir(upload_folder):
                ...
                if os.path.isfile(f_path):
                    os.remove(f_path)
            print("🧹 All uploaded PDFs deleted after embedding.")
        except Exception as e:
            print(f"⚠️ Could not clear uploaded_pdfs folder: {e}")

        # === 4️⃣ Response ===
        return JsonResponse({
            "status": "ok",
            "message": f"PDF '{file.name}' processed and deleted successfully."
        })
